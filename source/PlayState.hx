...
class PlayState extends MusicBeatState
{
	...
	var engineVer:FlxText;
	var timeSong:FlxText;
	...
	override public function create()
	{
				...
				case 'stagePixel':
					{
						...
						if (FlxG.save.data.antialiasing)
						{
							stageFrontpixel.antialiasing = false;
						}
						...
						if (FlxG.save.data.antialiasing)
						{
							stageCurtainspixel.antialiasing = false;
						}
		...
		// Engine Version
		engineVer = new FlxText(4,healthBarBG.y + 50,0, "Jloor Engine v1.0.5", 16);
		engineVer.setFormat(Paths.font("vcr.ttf"), 20, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		engineVer.cameras = [camHUD];
		engineVer.scrollFactor.set();
		add(engineVer);

		if (PlayStateChangeables.useDownscroll)
			engineVer.y = FlxG.height * 0.9 + 45;
		...
		if (FlxG.save.data.timeSong)
			{
				timeSong = new FlxText(0, strumLine.y - 100, FlxG.width, "0:00", 20);
				if (PlayStateChangeables.useDownscroll)
					timeSong.y -= 16; 
				timeSong.setFormat(Paths.font("vcr.ttf"), 42, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				timeSong.scrollFactor.set();
				timeSong.borderSize = 1.25;
				timeSong.screenCenter(X);
				timeSong.x -= 20;
				add(timeSong);
				timeSong.alpha = 0;
			}
	...
	function startCountdown():Void
	{
		...
		if (FlxG.save.data.timeSong)
		{
			FlxTween.tween(timeSong, {alpha: 1}, 1, {ease: FlxEase.cubeInOut});
			FlxTween.tween(timeSong, {y: timeSong.y + 80}, 1, {ease: FlxEase.backInOut});
		}
	...
	private function generateStaticArrows(player:Int, noteStyle:String):Void
	{
		for (i in 0...4)
		{
			...
			{
				case 'pixel':
					babyArrow.loadGraphic(Paths.image('notes/pixels'), true, 17, 17);
				...
				case 'guitar':
					babyArrow.frames = Paths.getSparrowAtlas('notes/guitar', 'shared');
				...
				case 'normal':
					babyArrow.frames = Paths.getSparrowAtlas('notes/normal', 'shared');
					babyArrow.animation.addByPrefix('green', 'arrowUP');
					babyArrow.animation.addByPrefix('blue', 'arrowDOWN');
					babyArrow.animation.addByPrefix('purple', 'arrowLEFT');
					babyArrow.animation.addByPrefix('red', 'arrowRIGHT');
			...
			babyArrow.alpha = 0;
			FlxTween.tween(babyArrow, {alpha: 1}, 1, {ease: FlxEase.cubeInOut});
			switch (player)
			{
				case 0:
	...
	override public function update(elapsed:Float)
	{
		...
		else
		{
			...
			if (!paused)
			{
				...
				if (FlxG.save.data.timeSong)
				{
					...
					timeSong.text = minutesRemaining + ':' + secondsRemaining;
				}
			}
		...
		if(FlxG.save.data.fullcombomode) { //Full Combo Mode Jijijija!
			if(misses == 1)
			{
		...
		if (generatedMusic)
			{
				...
				{	
					...
					if (!daNote.mustPress && daNote.wasGoodHit)
					{
						...
						if (FlxG.save.data.noteSplashdad)
							{
								if(!daNote.isSustainNote)
									{
	...
	private function popUpScore(daNote:Note):Void
		{
			...
			if (FlxG.save.data.noteSplashboyfriend)
				{
					if (daRating == 'sick'){
					var angles:Array<Int> = [25, 60, 180, 260, 0];
			...
			var timing:FlxSprite = new FlxSprite().loadGraphic(Paths.image(daTiming, 'shared'));
			timing.screenCenter();
			...
			if(!PlayStateChangeables.botPlay) add(timing);
